...
export default function CrearEventoScreen() {
  ...
  const selectLocation = item => {
    const trad = (item.traducnooficial && item.traducnooficial.toLowerCase()!=='null') ? item.traducnooficial + ' ' : '';
  ...
  const handleDateSelect = date => {
    ...
    if (!startDate || (startDate && endDate)) {
      setStartDate(sel.format('YYYY-MM-DD')); setEndDate('');
      setMarkedDates({ [sel.format('YYYY-MM-DD')]:{ startingDay:true, color:'#fd882d', textColor:'white' } });
    } else {
  ...
  return (
    <SafeAreaView style={styles.container}>
      {/* Header igual a Gestión de Falleros */}
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
          <Ionicons name="arrow-back" size={24} color="#fff" />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>{isEditMode?'Editar Evento':'Crear Evento'}</Text>
      </View>
      ...
      >
        <ScrollView contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
              ...
              {filteredLocations.map((it,i)=>(
                ...
                >
                  <Text style={styles.suggestionText}>
                    {`${it.codtipovia} ${it.traducnooficial!=='null'?it.traducnooficial+' ':''}${it.nomoficial}, ${calles.Municipio}`}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
          ...
          <View style={styles.dateTimeRow}>
            <TouchableOpacity style={[styles.iconButton,styles.flexItem]} onPress={()=>setShowStartTimePicker(true)}>
              <View style={styles.iconContent}>
                <Ionicons name="time-outline" size={20} color="#fff" />
                <Text style={styles.iconText}>Hora inicio: {moment(startTime).format('HH:mm')}</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.iconButton,styles.flexItem]} onPress={()=>setShowEndTimePicker(true)}>
              <View style={styles.iconContent}>
                <Ionicons name="time-outline" size={20} color="#fff" />
                <Text style={styles.iconText}>Hora fin: {moment(endTime).format('HH:mm')}</Text>
              </View>
            </TouchableOpacity>
          </View>
      ...
      <Modal visible={calendarVisible} animationType="slide">
        <View style={styles.calendarContainer}>
          <Calendar
            ...
            theme={{
              calendarBackground:'#121212',dayTextColor:'#fff',monthTextColor:'#fd882d',
              selectedDayBackgroundColor:'#fd882d',selectedDayTextColor:'#fff',todayTextColor:'#fd882d'
            }}
          />
          <TouchableOpacity onPress={()=>setCalendarVisible(false)} style={[styles.button,{marginTop:12}]}>            
      ...
      {showStartTimePicker && (
        <DateTimePicker
          ...
          is24Hour
          display="spinner"
          onChange={(e,sel)=>{setShowStartTimePicker(false);if(sel) setStartTime(sel);}}
        />
      )}
      {showEndTimePicker && (
        <DateTimePicker
          ...
          is24Hour
          display="spinner"
          onChange={(e,sel)=>{setShowEndTimePicker(false);if(sel) setEndTime(sel);}}
        />
      )}
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  input:               { backgroundColor:'#2c2c2c', color:'#fff', padding:12, borderRadius:8, marginBottom:12 },
  textArea:            { height:100, textAlignVertical:'top' },
  charCount:           { color:'#aaa', fontSize:12, textAlign:'right', marginBottom:8, marginTop:-8 },
  iconButton:          { backgroundColor:'#2c2c2c', padding:12, borderRadius:8, marginBottom:12, alignItems:'center', justifyContent:'center' },
  iconContent:         { flexDirection:'row', alignItems:'center', justifyContent:'center', flexWrap:'wrap' },
  iconText:            { color:'#fff', marginLeft:8, textAlign:'center' },
  button:              { backgroundColor:'#fd882d', padding:14, borderRadius:8, alignItems:'center', marginTop:12 },
  buttonText:          { color:'#fff', fontWeight:'bold' },
  suggestionBox:       { backgroundColor:'#1e1e1e', marginBottom:12, borderRadius:8, overflow:'hidden' },
